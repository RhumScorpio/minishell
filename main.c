...
/*   Updated: 2023/02/28 09:56:13 by clesaffr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	put_syntax_error(t_env *envl, char *error)
{
	free(envl->value);
	envl->value = ft_strdup(error);
	return (1);
}
...
int	minishell(t_tok *tokenl, t_exec *exec, t_env *envl, char *line_read)
{
	...
	if (ft_check_tokenl(tokenl) == 1)
	{
		...
		free(tokenl);
		return (put_syntax_error(envl, "2"));
	}
	if (ft_prep_exec(tokenl, exec))
	{
		destroy_tokenl(tokenl);
		ft_start_exec(exec, envl);
	}
	else
	{
		put_syntax_error(envl, "1");
		destroy_tokenl(tokenl);
	}
